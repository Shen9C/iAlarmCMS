...
class Task(db.Model):
    ...
    created_at = db.Column(TIMESTAMP(timezone=True), default=lambda: datetime.now(datetime.timezone.utc), comment='创建时间')
    updated_at = db.Column(TIMESTAMP(timezone=True), default=lambda: datetime.now(datetime.timezone.utc), onupdate=lambda: datetime.now(datetime.timezone.utc), comment='修改时间')
