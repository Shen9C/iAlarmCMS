...
# 预置默认配置
DEFAULT_CONFIG = {
    'url': 'http://localhost:5566',  # 更新为新的API端口
...
class EdgeDeviceApiTester:
    ...
    def __init__(self, base_url, device_id=None, secret_key=None):
        """初始化测试工具
    ...
    def get_token(self):
        """获取设备认证令牌"""
        print("\n===== 测试设备认证获取Token =====")
        url = f"{self.base_url}/api/devices/auth/token"  # 路径保持不变
        payload = {
    ...
    def upload_alarm(self, alarm_type, alarm_code, well_code=None, well_name=None, description=None):
        ...
        url = f"{self.base_url}/api/devices/alarms"  # 路径保持不变
        ...
        print(f"请求URL: {url}")
        print(f"请求头: Authorization: Bearer {self.token[:20]}...")
        print(f"请求参数: {json.dumps(payload, ensure_ascii=False)}")
...
def main():
    ...
    parser.add_argument('--url', help='API服务器地址，例如 http://localhost:5000')
